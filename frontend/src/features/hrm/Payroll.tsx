...
const statusColors = { pending: 'bg-yellow-100 text-yellow-700', paid: 'bg-green-100 text-green-700', hold: 'bg-red-100 text-red-500' };
  ...
  return (
    <div>
      <PageHeader
        ...
        breadcrumbs={[{ label: 'Home', path: '/admin' }, { label: 'HRM', path: '/admin/hrm/employees' }, { label: 'Payroll' }]}
        actions={
          <button onClick={() => toast.success('Payroll generation coming soon')} className="flex items-center gap-2 px-4 py-2 bg-[#ff6d29] text-white rounded-lg text-sm font-medium hover:bg-[#e65a1f]">
            <CheckCircle className="h-4 w-4" /> Generate Payroll
          </button>
        }
      />

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-white border border-[#DBDFE9] rounded-lg p-4"><p className="text-xs text-gray-500">Total Payroll</p><p className="text-2xl font-bold mt-1">৳{totalPayroll.toLocaleString()}</p></div>
        <div className="bg-white border border-[#DBDFE9] rounded-lg p-4"><p className="text-xs text-gray-500">Paid</p><p className="text-2xl font-bold text-green-600 mt-1">৳{paidAmount.toLocaleString()}</p></div>
        <div className="bg-white border border-[#DBDFE9] rounded-lg p-4"><p className="text-xs text-gray-500">Pending</p><p className="text-2xl font-bold text-yellow-600 mt-1">৳{(totalPayroll - paidAmount).toLocaleString()}</p></div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-[#DBDFE9]">
                {['Employee', 'Month', 'Basic', 'Allowances', 'Deductions', 'Net Salary', 'Status', 'Action'].map((h) => (
                    ...
                    <div className="flex gap-1.5">
                      ...
                      {item.status === 'pending' && (
                        <button onClick={() => { setRecords((prev) => prev.map((r) => r.id === item.id ? { ...r, status: 'paid' } : r)); toast.success('Marked as paid'); }}
                          className="px-2 py-1 bg-green-100 text-green-700 rounded text-xs hover:bg-green-200">Pay</button>
                      )}
                    </div>
                  </td>
                </tr>
