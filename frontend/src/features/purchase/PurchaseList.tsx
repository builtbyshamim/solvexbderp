...
const PurchaseList = () => {
  ...
  return (
    <div>
      <PageHeader
        title="Purchase List"
        subtitle="View and manage all purchase invoices"
        breadcrumbs={[{ label: 'Home', path: '/admin' }, { label: 'Purchase' }, { label: 'Purchase List' }]}
        actions={
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {[
          { label: 'Total Purchase', value: `৳${totalPurchase.toLocaleString()}`, cls: 'text-[#26272F]' },
          { label: 'Total Paid', value: `৳${totalPaid.toLocaleString()}`, cls: 'text-green-600' },
      ...
      <div className="bg-white border border-[#DBDFE9] rounded-lg shadow-sm">
        <div className="p-4 border-b border-[#DBDFE9] flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1 sm:max-w-xs">
            ...
            <input type="text" placeholder="Search invoice or supplier..." value={search} onChange={(e) => setSearch(e.target.value)}
          ...
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}
            ...
            {['All', 'Received', 'Partial', 'Pending', 'Returned'].map((s) => <option key={s}>{s}</option>)}
          </select>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-[#DBDFE9]">
                {['Invoice No', 'Date', 'Supplier', 'Warehouse', 'Total', 'Paid', 'Due', 'Status', 'Action'].map((h) => (
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.length === 0 ? (
                <tr><td colSpan={9} className="px-4 py-12 text-center text-gray-400">No purchases found</td></tr>
              ) : (
                filtered.map((item) => (
