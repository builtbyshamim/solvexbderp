      ...
      <div className="bg-white border border-[#DBDFE9] rounded-lg p-4 mb-5">
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center flex-wrap">
          ...
          <select
            ...
            className="w-full sm:w-64 px-3 py-2 border border-[#DBDFE9] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#ff6d29]/20 focus:border-[#ff6d29]"
          >
          ...
          {selectedSupplier && (
            <button onClick={openAdj}
              className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-lg text-sm hover:bg-purple-700 transition-colors sm:ml-auto">
        ...
        {selectedSupplier && (
          <div className="mt-3 pt-3 border-t border-gray-100 flex flex-wrap gap-4 text-sm">
            <div>
              ...
              <span className="font-semibold text-[#26272F]">{selectedSupplier.name}</span>
            </div>
            <div>
              <span className="text-xs text-gray-400 block">Opening Balance</span>
              <span className="font-semibold text-[#26272F]">৳{fmt(selectedSupplier.openingBalance)}</span>
            </div>
            <div>
              <span className="text-xs text-gray-400 block">Current Balance</span>
              <span className={`font-bold ${Number(selectedSupplier.currentBalance) > 0 ? 'text-red-500' : 'text-green-600'}`}>
                ৳{fmt(selectedSupplier.currentBalance)}
              </span>
            </div>
          </div>
        )}
      ...
      {!selectedSupplierId ? (
        ...
        <div className="bg-white border border-[#DBDFE9] rounded-lg shadow-sm">
          <div className="px-4 py-3 border-b border-[#DBDFE9] flex items-center justify-between">
            ...
            <span className="text-xs text-gray-500">{meta.totalItems} entries</span>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {loadingLedger ? (
                  ...
                  <tr><td colSpan={7} className="px-4 py-12 text-center text-gray-400">No ledger entries found</td></tr>
                  ...
                  <tr key={entry.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-red-500 font-medium text-xs">
                      {Number(entry.debit ?? 0) > 0 ? `৳${fmt(entry.debit)}` : '—'}
                    </td>
                    <td className="px-4 py-3 text-green-600 font-medium text-xs">
                      {Number(entry.credit ?? 0) > 0 ? `৳${fmt(entry.credit)}` : '—'}
                    </td>
                    <td className="px-4 py-3 font-bold text-[#26272F] text-xs">
                      ৳{fmt(entry.balanceAfter ?? entry.balance ?? 0)}
                    </td>
          ...
          <div className="md:hidden divide-y divide-gray-100">
            {loadingLedger ? (
              ...
              <div className="py-12 text-center text-gray-400 text-sm">No ledger entries found</div>
            ) : entries.map((entry: any) => (
              <div key={entry.id} className="p-4 space-y-2.5">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium capitalize shrink-0 ${typeColors[entry.transactionType ?? entry.type] ?? 'bg-gray-100 text-gray-600'}`}>
                    {entry.transactionType ?? entry.type ?? '—'}
                  </span>
                </div>
                {(entry.referenceId ?? entry.reference) && (
                  <p className="text-xs font-mono text-[#ff6d29]">{entry.referenceId ?? entry.reference}</p>
                )}
                <div className="grid grid-cols-3 gap-2">
                  <div className="bg-red-50 rounded-lg p-2 text-center">
                    <p className="text-[10px] text-gray-400 mb-0.5">Debit</p>
                    <p className="text-xs font-semibold text-red-500">{Number(entry.debit ?? 0) > 0 ? `৳${fmt(entry.debit)}` : '—'}</p>
                  </div>
                  <div className="bg-green-50 rounded-lg p-2 text-center">
                    <p className="text-[10px] text-gray-400 mb-0.5">Credit</p>
                    <p className="text-xs font-semibold text-green-600">{Number(entry.credit ?? 0) > 0 ? `৳${fmt(entry.credit)}` : '—'}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-2 text-center">
                    <p className="text-[10px] text-gray-400 mb-0.5">Balance</p>
                    <p className="text-xs font-bold text-[#26272F]">৳{fmt(entry.balanceAfter ?? entry.balance ?? 0)}</p>
                  </div>
                </div>
                  ...
                  {(['debit', 'credit'] as const).map((tp) => (
                    <button key={tp} type="button" onClick={() => setAdjForm((f) => ({ ...f, type: tp }))}
                      className={`py-2.5 rounded-lg border text-sm font-medium transition-colors ${
                        adjForm.type === tp
                          ? tp === 'debit' ? 'border-red-500 bg-red-50 text-red-600' : 'border-green-500 bg-green-50 text-green-600'
                          : 'border-[#DBDFE9] text-gray-500 hover:bg-gray-50'
                      }`}>
                      {tp === 'debit' ? '▲ Debit (owe more)' : '▼ Credit (owe less)'}
                    </button>
                  ))}
                </div>
              </div>
