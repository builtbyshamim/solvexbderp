...
const AllAccounts = () => {
  ...
  const tableHeaders = [
    t('accounting.accounts.accountName'),
    t('common.type'),
    t('accounting.accounts.accountNumber'),
    t('accounting.accounts.openingBalance'),
    t('accounting.accounts.currentBalance'),
    t('common.actions'),
  ];
    ...
    <div>
      ...
      <div className="bg-white border border-[#DBDFE9] rounded-lg shadow-sm">
        <div className="p-4 border-b border-[#DBDFE9]">
          <h3 className="font-semibold text-[#26272F]">{t('accounting.accounts.listTitle')} ({accounts.length})</h3>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr><td colSpan={6} className="px-4 py-12 text-center text-gray-400">{t('accounting.accounts.noAccounts')}</td></tr>
              ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            <h2 className="text-lg font-semibold text-[#26272F] mb-4">
              {editItem ? t('accounting.accounts.editAccount') : t('accounting.accounts.addAccount')}
            </h2>
            <div className="space-y-3">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {t('accounting.accounts.accountName')} <span className="text-red-500">*</span>
                </label>
              ...
              {!editItem && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t('accounting.accounts.accountType')}</label>
                  <select value={form.accountType} onChange={(e) => f('accountType', e.target.value)}
                    className="w-full px-3 py-2 border border-[#DBDFE9] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#ff6d29]/20 focus:border-[#ff6d29]">
                    <option value="cash">{t('accounting.accounts.cashOption')}</option>
                    <option value="bank">{t('accounting.accounts.bankOption')}</option>
                    <option value="mobile_banking">{t('accounting.accounts.mobileBankingOption')}</option>
                  </select>
                </div>
              )}
              {!editItem && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t('accounting.accounts.openingBalance')}</label>
                  <input type="number" min="0" value={form.openingBalance} onChange={(e) => f('openingBalance', e.target.value)} placeholder="0"
                    className="w-full px-3 py-2 border border-[#DBDFE9] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#ff6d29]/20 focus:border-[#ff6d29]" />
                </div>
              )}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('accounting.accounts.bankName')}</label>
                <input value={form.bankName} onChange={(e) => f('bankName', e.target.value)} placeholder="e.g. Dutch Bangla Bank"
                  className="w-full px-3 py-2 border border-[#DBDFE9] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#ff6d29]/20 focus:border-[#ff6d29]" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('accounting.accounts.accountNumber')}</label>
            ...
            <div className="flex gap-3 mt-5 justify-end">
              ...
              <button onClick={handleSave} disabled={creating || updating}
                ...
                {editItem ? t('common.update') : t('accounting.accounts.addAccount')}
              </button>
            </div>
          </div>
        </div>
      ...
      {deleteItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-6 text-center">
            ...
            <p className="text-sm text-gray-500 mb-6">{t('accounting.accounts.deleteWarning')}</p>
