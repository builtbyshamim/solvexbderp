...
import { Search, Plus, Trash2, Loader2, TrendingDown, Settings2, X, Pencil, Check } from 'lucide-react';
...
const Expenses = () => {
  ...
  const { data: categories = [] } = useGetAccountingCategoriesQuery({ type: 'expense' });
  ...
  const handleCreateCat = async () => {
    ...
    try {
      await createCategory({ name: newCatName.trim(), type: 'expense', color: newCatColor }).unwrap();
  ...
  // summary cards: top 4 categories by spend in current view
  const catSummary = (categories as any[])
    .map((c) => ({
      cat: c,
      amt: expenses.filter((e: any) => e.category === c.name).reduce((s: number, e: any) => s + Number(e.credit), 0),
    }))
    .filter((x) => x.amt > 0)
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={t('accounting.expenses.subtitle')}
        breadcrumbs={[
          { label: t('common.home'), path: '/admin' },
          { label: t('nav.accounting'), path: '/admin/accounting/accounts' },
          { label: t('accounting.expenses.title') },
        ]}
        actions={
          <div className="flex gap-2">
            ...
            >
              <Plus className="h-4 w-4" /> {t('accounting.expenses.addExpense')}
            </button>
          </div>
        }
      />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          {catSummary.map(({ cat, amt }) => (
            <button
              ...
              className={`text-left p-4 rounded-xl border-2 transition-all ${
                filterCategory === cat.name
                  ...
                  : 'bg-white border-[#DBDFE9] hover:border-gray-300'
              }`}
            ...
            >
              ...
              <p className="text-xl font-bold text-red-500">৳{amt.toLocaleString('en-BD')}</p>
            </button>
          ))}
        </div>
      )}
        ...
        <div className="p-4 border-b border-[#DBDFE9] flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-red-500">
              {t('common.total')}: ৳{totalExpense.toLocaleString('en-BD')}
            </span>
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr><td colSpan={6} className="px-4 py-12 text-center text-gray-400">{t('accounting.expenses.noRecords')}</td></tr>
              ) : (
                expenses.map((item: any) => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3 font-bold text-red-600">৳{Number(item.credit).toLocaleString('en-BD')}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <h2 className="text-base font-semibold text-[#26272F]">{t('accounting.expenses.recordExpense')}</h2>
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <CategorySelect type="expense" value={form.category} onChange={(v) => f('category', v)} />
              </div>
              <div>
            ...
            <div className="flex gap-3 p-5 border-t border-gray-100 justify-end">
              ...
              <button onClick={handleSave} disabled={saving}
                ...
                {saving && <Loader2 className="h-4 w-4 animate-spin" />} {t('accounting.expenses.saveExpense')}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white w-full max-w-sm h-full shadow-2xl flex flex-col">
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <h2 className="text-base font-semibold text-[#26272F] flex items-center gap-2">
                <Settings2 className="h-4 w-4 text-[#ff6d29]" /> Expense Categories
              </h2>
      ...
      {deleteItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">{t('accounting.expenses.deleteTitle')}</h3>
            <p className="text-sm text-gray-500 mb-6">{t('accounting.expenses.deleteWarning')}</p>
