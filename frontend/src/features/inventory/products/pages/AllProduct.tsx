    ...
    <div>
      ...
      <div className="bg-white border border-[#DBDFE9] rounded-lg shadow-sm">
        <div className="p-4 border-b border-[#DBDFE9] flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          ...
          <span className="text-sm text-gray-500">{meta.totalItems} {t('inventory.products.title').toLowerCase()}</span>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {isFetching ? (
                <tr><td colSpan={10} className="px-4 py-12 text-center text-gray-400">{t('inventory.products.loading')}</td></tr>
              ) : products.length === 0 ? (
                <tr><td colSpan={10} className="px-4 py-16 text-center">
                  <Package className="h-12 w-12 mx-auto mb-3 text-gray-200" />
                  <p className="text-gray-400 font-medium">{t('inventory.products.noProducts')}</p>
                    ...
                    <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${product.hasVariants ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'}`}>
                          {product.hasVariants ? t('inventory.products.variantYes') : t('inventory.products.variantNo')}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${product.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-500'}`}>
                          {product.isActive ? t('inventory.products.statusActive') : t('inventory.products.statusInactive')}
                        </span>
                      </td>
                      <td className="px-4 py-3">
