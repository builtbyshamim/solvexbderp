...
const AllCategory = () => {
  ...
  const { data, error, isFetching, refetch } = useGetAllCategoryQuery({
    ...searchValue,
    ...
    <div>
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: t('inventory.categories.title') },
        ]}
        actions={
          <button
      ...
      <div className="bg-white border border-[#DBDFE9] rounded-lg shadow-sm">
        <div className="p-4 border-b border-[#DBDFE9] flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          ...
          <span className="text-sm text-gray-500">
            {meta.totalItems} {t('inventory.categories.categoriesCount')}
          </span>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-[#DBDFE9]">
                  {[
                    '#',
                    t('inventory.categories.colImage'),
                    t('common.name'),
                    t('common.status'),
                    t('common.action'),
                  ].map((h) => (
                ...
                {isFetching ? (
                  ...
                  categories.map((cat: any, index: number) => (
                    <tr key={cat._id || cat.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`px-2.5 py-1 rounded-full text-xs font-medium ${cat.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}
                        >
                          {cat.isActive
      ...
      <CommonModal
        ...
        title={t('inventory.categories.editTitle')}
      >
