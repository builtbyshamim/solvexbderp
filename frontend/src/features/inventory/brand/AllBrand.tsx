  ...
  return (
    <div>
      <PageHeader
        ...
        breadcrumbs={[{ label: 'Home', path: '/admin' }, { label: 'Inventory', path: '/admin/inventory/products' }, { label: 'Brands' }]}
        actions={
          <button
      ...
      <div className="bg-white border border-[#DBDFE9] rounded-lg shadow-sm">
        <div className="p-4 border-b border-[#DBDFE9] flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          ...
          <span className="text-sm text-gray-500">{meta.totalItems} brands</span>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-[#DBDFE9]">
                  {['#', 'Logo', 'Name', 'Status', 'Actions'].map((h) => (
                    ...
                    <tr key={brand._id || brand.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${brand.status === 'active' || brand.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                          {brand.status === 'active' || brand.isActive ? 'Active' : 'Inactive'}
                        </span>
                      </td>
                      <td className="px-4 py-3">
      ...
      <CommonModal isOpen={addOpen} onClose={() => setAddOpen(false)} title="Add New Brand">
        <AddBrand onClose={() => setAddOpen(false)} />
      </CommonModal>

      <CommonModal isOpen={!!editItem} onClose={() => setEditItem(false)} title="Edit Brand">
