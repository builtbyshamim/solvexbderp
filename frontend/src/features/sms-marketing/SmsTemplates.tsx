    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white border border-[#DBDFE9] rounded-xl p-16 text-center">
          ...
          <p className="text-gray-400 text-sm mt-1">Create your first SMS template to get started</p>
          <button
            onClick={openCreate}
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {templates.map((t: any) => (
              <div key={t.id} className="bg-white border border-[#DBDFE9] rounded-xl shadow-sm hover:shadow-md transition-shadow flex flex-col">
                <div className="p-5 flex-1">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <h3 className="font-semibold text-[#26272F] text-sm leading-tight">{t.name}</h3>
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize shrink-0 ${categoryColors[t.category] || 'bg-gray-100 text-gray-600'}`}>
                      {t.category}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-3 mt-3 text-xs text-gray-400">
                    ...
                    <span>·</span>
                    <span>{Math.ceil(t.characterCount / CHAR_LIMIT)} part{Math.ceil(t.characterCount / CHAR_LIMIT) > 1 ? 's' : ''}</span>
                    {t.usageCount > 0 && (
                      <>
                        <span>·</span>
          ...
          {meta && meta.totalPages > 1 && (
            <div className="flex items-center justify-between mt-6 text-sm text-gray-500">
              <span>{meta.totalItems} templates</span>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="p-5 border-b border-gray-100 flex items-center justify-between">
              <h2 className="font-bold text-[#26272F]">{editId ? 'Edit Template' : 'New Template'}</h2>
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="block text-xs font-medium text-gray-600">Message Content <span className="text-red-500">*</span></label>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h2 className="font-bold text-[#26272F]">Delete Template?</h2>
            </div>
