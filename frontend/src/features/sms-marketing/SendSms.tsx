...
const SendSms = () => {
  ...
  return (
    <div>
      <PageHeader
        title="Send SMS"
        subtitle="Send instant SMS to individual or multiple recipients"
        breadcrumbs={[
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-white border border-[#DBDFE9] rounded-xl shadow-sm">
            <div className="p-4 border-b border-[#DBDFE9] flex items-center gap-2">
              ...
              <h2 className="font-semibold text-[#26272F]">Recipients</h2>
              {phones.length > 0 && (
            ...
            <div className="p-4 space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Recipient Name <span className="text-gray-400">(optional)</span></label>
          ...
          <div className="bg-white border border-[#DBDFE9] rounded-xl shadow-sm">
            <div className="p-4 border-b border-[#DBDFE9] flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold text-[#26272F]">Message</h2>
              </div>
            ...
            <div className="p-4 space-y-3">
              {templates.length > 0 && (
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Use Template</label>
          ...
          >
            ...
            {isLoading ? 'Sending...' : `Send to ${phones.length || 0} Recipient${phones.length !== 1 ? 's' : ''}`}
          </button>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white border border-[#DBDFE9] rounded-xl shadow-sm">
            <div className="p-4 border-b border-[#DBDFE9]">
              <h2 className="font-semibold text-[#26272F] text-sm">Preview</h2>
            </div>
            <div className="p-4">
          ...
          <div className="bg-white border border-[#DBDFE9] rounded-xl shadow-sm">
            <div className="p-4 border-b border-[#DBDFE9]">
              <h2 className="font-semibold text-[#26272F] text-sm">Summary</h2>
            </div>
            <div className="p-4 space-y-3 text-sm">
              {[
                { label: 'Recipients', value: phones.length },
                { label: 'SMS Parts', value: parts },
                { label: 'Total SMS', value: phones.length * parts },
              ].map((r) => (
                <div key={r.label} className="flex justify-between items-center text-gray-600">
                  <span>{r.label}</span>
          ...
          {result && (
            <div className="bg-green-50 border border-green-200 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-semibold text-green-700 text-sm">Sent Successfully</span>
              </div>
              <div className="space-y-1.5 text-sm">
                <div className="flex justify-between text-gray-600">
                  <span>Total</span><span className="font-medium">{result.total}</span>
                </div>
                <div className="flex justify-between text-green-600">
                  <span>Sent</span><span className="font-medium">{result.sent}</span>
                </div>
                {result.failed > 0 && (
                  <div className="flex justify-between text-red-600">
                    <span>Failed</span><span className="font-medium">{result.failed}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-xs text-blue-700 space-y-1.5">
            ...
            <p>• Keep messages under 160 chars to avoid extra costs</p>
            <p>• Use templates for consistent messaging</p>
            <p>• Add multiple numbers with commas</p>
            <p>• Use variables: {'{name}'}, {'{amount}'}, {'{date}'}</p>
          </div>
        </div>
      </div>
    </div>
