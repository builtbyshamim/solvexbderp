...
const SmsCampaigns = () => {
  ...
  return (
    <div>
      <PageHeader
        title="SMS Campaigns"
        subtitle="Create and manage bulk SMS campaigns"
        breadcrumbs={[
          ...
          <>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-[#DBDFE9]">
                    {['Campaign', 'Target', 'Sent', 'Failed', 'Rate', 'Status', 'Date', 'Actions'].map(h => (
                      ...
                      <tr key={c.id} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1 text-gray-600">
                            ...
                            <span className="text-xs capitalize">{c.targetType === 'all_customers' ? 'All Customers' : c.targetType === 'group' ? 'Group' : 'Specific'}</span>
                          </div>
                          {c.totalCount > 0 && <p className="text-xs text-gray-400 mt-0.5">{c.totalCount} recipients</p>}
                        </td>
                        <td className="px-4 py-3">
            ...
            {meta && meta.totalPages > 1 && (
              <div className="p-4 border-t border-[#DBDFE9] flex items-center justify-between text-sm text-gray-500">
                <span>{meta.totalItems} campaigns</span>
                <div className="flex items-center gap-2">
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
            <div className="p-5 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white">
              <h2 className="font-bold text-[#26272F]">{editId ? 'Edit Campaign' : 'New Campaign'}</h2>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600 text-xl leading-none">×</button>
            </div>
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  <label className="block text-xs font-medium text-gray-600 mb-1">Campaign Name <span className="text-red-500">*</span></label>
              ...
              {templates.length > 0 && (
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Use Template <span className="text-gray-400">(optional)</span></label>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Target Audience <span className="text-red-500">*</span></label>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { value: 'all_customers', label: 'All Customers', icon: <Users className="h-4 w-4" /> },
                    { value: 'group', label: 'Group', icon: <Users className="h-4 w-4" /> },
                    { value: 'specific_numbers', label: 'Specific Nos.', icon: <Send className="h-4 w-4" /> },
                  ].map(opt => (
              ...
              {form.targetType === 'group' && (
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Select Group <span className="text-red-500">*</span></label>
              ...
              {form.targetType === 'specific_numbers' && (
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Phone Numbers <span className="text-red-500">*</span></label>
                  <textarea rows={3} placeholder="01700000000, 01800000000, 01900000000"
                    ...
                    className="w-full px-3 py-2 border border-[#DBDFE9] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#ff6d29]/20 focus:border-[#ff6d29] resize-none" />
                  <p className="text-xs text-gray-400 mt-1">Separate numbers with commas</p>
                </div>
              )}

              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Schedule <span className="text-gray-400">(optional — leave empty to save as draft)</span></label>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h2 className="font-bold text-[#26272F]">Send Campaign?</h2>
            </div>
            <p className="text-sm text-gray-500 mb-5">
              This will immediately send SMS to all targeted recipients. This action cannot be undone.
            </p>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h2 className="font-bold text-[#26272F]">Delete Campaign?</h2>
            </div>
