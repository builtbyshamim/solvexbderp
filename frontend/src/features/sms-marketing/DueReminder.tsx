...
const DueReminder = () => {
  ...
  return (
    <div>
      <PageHeader
        title="Due Reminders"
        subtitle="Automatically notify customers with outstanding balances"
        breadcrumbs={[
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 mb-6 flex items-start gap-3">
        ...
        <div className="text-sm text-blue-700">
          <p className="font-semibold">How Due Reminders Work</p>
          <p className="text-xs text-blue-600 mt-0.5">Create reminder rules with custom messages and minimum due amounts. Trigger manually or set them to run automatically. Variables like <code className="bg-blue-100 px-1 rounded">{'{name}'}</code> and <code className="bg-blue-100 px-1 rounded">{'{amount}'}</code> are replaced with actual customer data.</p>
        </div>
      </div>
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl w-fit mb-6">
        {(['rules', 'logs'] as const).map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            ...
            {tab === 'rules' ? (
              <span className="flex items-center gap-1.5"><Bell className="h-4 w-4" /> Reminder Rules</span>
            ) : (
              <span className="flex items-center gap-1.5"><Clock className="h-4 w-4" /> Send History</span>
            )}
          </button>
        ))}
      </div>
        ...
        <>
          {isLoading ? (
            ...
            <div className="bg-white border border-[#DBDFE9] rounded-xl p-16 text-center">
              ...
              <p className="font-semibold text-[#26272F]">No reminder rules yet</p>
              <p className="text-gray-400 text-sm mt-1">Create your first due reminder to start notifying customers</p>
              <button onClick={openCreate}
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-start justify-between gap-3 mb-3">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <button onClick={() => handleToggle(r.id)}
                          className={`p-1.5 rounded-lg transition-colors ${r.isActive ? 'text-green-600 hover:bg-green-50' : 'text-gray-400 hover:bg-gray-100'}`} title="Toggle">
                        ...
                        <button onClick={() => setSendId(r.id)}
                          className="p-1.5 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition-colors" title="Send Now">
                    ...
                    <div className="flex items-center gap-4 text-xs text-gray-500">
                      ...
                      <span className="flex items-center gap-1">
                        ...
                        {r.sendToAllDue ? 'All due customers' : 'Filtered customers'}
                      </span>
                      {r.totalSent > 0 && (
                    ...
                    {r.lastRunAt && (
                      <p className="text-xs text-gray-400 mt-2 flex items-center gap-1">
                        <Clock className="h-3 w-3" /> Last run: {new Date(r.lastRunAt).toLocaleString()}
                      </p>
                    )}
                  </div>
                </div>
          ...
          <div className="bg-white border border-[#DBDFE9] rounded-xl shadow-sm mb-4">
            <div className="p-4 flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input type="text" placeholder="Search customer..." value={logSearch}
          ...
          <div className="bg-white border border-[#DBDFE9] rounded-xl shadow-sm">
            {logsLoading ? (
              ...
              <div className="p-16 text-center">
                ...
                <p className="text-gray-500 font-medium">No reminder logs yet</p>
                <p className="text-gray-400 text-sm mt-1">Sent reminders will appear here</p>
              </div>
            ) : (
              <>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-gray-50 border-b border-[#DBDFE9]">
                        {['Customer', 'Phone', 'Due Amount', 'Reminder', 'Status', 'Sent At'].map(h => (
                ...
                {logMeta && logMeta.totalPages > 1 && (
                  <div className="p-4 border-t border-[#DBDFE9] flex items-center justify-between text-sm text-gray-500">
                    <span>{logMeta.totalItems} records</span>
                    <div className="flex items-center gap-2">
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
            <div className="p-5 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white">
              <h2 className="font-bold text-[#26272F]">{editId ? 'Edit Reminder' : 'New Due Reminder'}</h2>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600 text-xl">×</button>
            </div>
            <div className="p-5 space-y-4">
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Reminder Name <span className="text-red-500">*</span></label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Overdue Days</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Min Due Amount (৳)</label>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="block text-xs font-medium text-gray-600">Message <span className="text-red-500">*</span></label>
                ...
                <p className="text-xs text-gray-400 mt-1.5">Click variables above to insert them into your message</p>
              </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h2 className="font-bold text-[#26272F]">Send Reminder Now?</h2>
            </div>
            <p className="text-sm text-gray-500 mb-5">
              This will send the due reminder SMS to all qualifying customers immediately.
            </p>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h2 className="font-bold text-[#26272F]">Delete Reminder?</h2>
            </div>
