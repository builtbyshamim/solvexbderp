...
import { Plus, Trash2, Save, ArrowLeft, Search, Loader2, Percent, BadgeDollarSign, Truck, Tag } from 'lucide-react';
...
const ProductSearch = ({
  productName, warehouseId,
  onSelect,
}: {
  productName: string; warehouseId: string;
          ...
          {products.map((p: any) => {
            ...
            return (
              <button key={p.id} onMouseDown={() => {
                onSelect({ id: p.id, name: p.name, sellingPrice: Number(p.sellingPrice), stock: Number(stock) });
              ...
              >
                ...
                <div className="flex items-center justify-between text-xs mt-0.5">
                  ...
                  <span className={`font-semibold ${
                    ...
                    Number(stock) <= 5 ? 'text-yellow-600' : 'text-green-600'
                  }`}>
                    {Number(stock) <= 0 ? '✕ Out of stock' : `● ${Number(stock).toFixed(0)} in stock`}
                  </span>
                </div>
              </button>
            );
...
const AddSale = () => {
  ...
  const [items, setItems] = useState<SaleItem[]>([{
    id: 1, productId: '', productName: '', qty: 1, unitPrice: 0,
  ...
  const addPayment = () => setPayments((prev) => [
    ...prev, { id: Date.now(), method: 'cash', amount: '' },
  ]);
  ...
  const fillRemaining = () => {
    ...
    setPayments((prev) => prev.map((p, i) =>
      i === prev.length - 1 ? { ...p, amount: remaining.toFixed(2) } : p
    ));
  };
  ...
  const handleSubmit = async () => {
    ...
    if (overStock) {
      toast.error(`"${overStock.productName}" — only ${overStock.maxStock} units available, you entered ${overStock.qty}`);
      return;
    }
    ...
    try {
      const payload: any = {
        ...
        payments: validPayments.length > 0
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-white border border-[#DBDFE9] rounded-lg p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1.5">Invoice No <span className="text-gray-400">(optional, auto-generated)</span></label>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {items.map((item) => {
                    ...
                    const nearLimit = stockKnown && !overLimit && item.qty >= Math.ceil(item.maxStock * 0.8);
                    const noStock   = stockKnown && item.maxStock <= 0;
                    return (
                    <tr key={item.id} className={`border-b border-gray-50 ${overLimit || noStock ? 'bg-red-50/30' : ''}`}>
                          ...
                          <div className="mt-1 flex items-center gap-1">
                            <span className={`inline-flex items-center gap-1 text-xs font-medium ${
                              ...
                              {noStock   ? 'Out of stock' :
                               overLimit ? `Only ${item.maxStock} in stock` :
                               nearLimit ? `${item.maxStock} in stock (low)` :
                      ...
                      <td className="py-2 pr-3">
                        <input
                          ...
                          className={`${inp} w-16 text-center ${
                            overLimit || noStock ? 'border-red-400 bg-red-50 text-red-700 font-semibold' :
                            nearLimit ? 'border-yellow-400' : ''
                          }`}
                        />
                        {stockKnown && item.maxStock > 0 && !overLimit && (
                          <p className={`text-xs mt-0.5 text-center ${nearLimit ? 'text-yellow-600 font-medium' : 'text-gray-400'}`}>
                            {item.maxStock - item.qty === 0 ? 'Max!' : `${item.maxStock - item.qty} left`}
                          </p>
                        )}
                        {noStock && (
                          <p className="text-xs mt-0.5 text-center text-red-600 font-medium">No stock</p>
                        )}
                      </td>
                      <td className="py-2 pr-3">
                      ...
                      <td className="py-2 pr-3">
                        <div className="flex items-center gap-1">
                          ...
                          <button
                            ...
                            title={item.discountType === 'flat' ? 'Switch to percent' : 'Switch to flat'}
                          >
                            {item.discountType === 'percent' ? '%' : '৳'}
                          </button>
                          <input
          ...
          <div className="bg-white border border-[#DBDFE9] rounded-lg p-5">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex border border-[#DBDFE9] rounded overflow-hidden text-xs">
                    ...
                    <button type="button"
                      onClick={() => setForm({ ...form, discountType: 'percent', discountValue: '' })}
            ...
            {/* Note */}
            <div className="mt-4">
              <label className="block text-xs font-medium text-gray-600 mb-1.5">Note <span className="text-gray-400">(optional)</span></label>
          ...
          <div className="bg-white border border-[#DBDFE9] rounded-lg p-5 sticky top-4 space-y-4">
            ...
            <div>
              ...
              <div className="space-y-1.5 text-sm">
                ...
                {orderDiscount > 0 && (
                  <div className="flex justify-between text-green-600">
                    <span>Order Discount{form.discountType === 'percent' && ` (${form.discountValue}%)`}</span>
            ...
            <div>
              ...
              <div className="flex gap-2 mt-2">
                <button type="button"
                  onClick={() => setPayments((prev) => prev.map((p, i) =>
                    i === 0 ? { ...p, amount: grandTotal.toFixed(2) } : p
                  ))}
