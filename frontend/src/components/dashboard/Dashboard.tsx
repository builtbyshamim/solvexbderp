...
const Dashboard = () => {
  ...
  const statCards = [
    { labelKey: 'dashboard.todaySales',    value: '৳48,250',   change: '+12.4%',                             up: true,  icon: ShoppingCart, color: 'bg-[#fff3eb] text-[#ff6d29]',   link: '/admin/sales/list' },
    { labelKey: 'dashboard.receivableDue', value: '৳1,24,500', change: `18 ${t('dashboard.customers')}`,    up: null,  icon: CreditCard,   color: 'bg-blue-50 text-blue-600',       link: '/admin/sales/customer-ledger' },
    { labelKey: 'dashboard.payableDue',    value: '৳87,000',   change: `7 ${t('dashboard.suppliers')}`,     up: null,  icon: Wallet,       color: 'bg-purple-50 text-purple-600',   link: '/admin/purchase/supplier-ledger' },
    { labelKey: 'dashboard.lowStockAlerts',value: '12 Items',  change: t('dashboard.needsReorder'),         up: false, icon: AlertTriangle, color: 'bg-red-50 text-red-500',        link: '/admin/inventory/stock-report' },
  ];

  const quickStats = [
    { labelKey: 'dashboard.totalProducts',    value: '124',   icon: Package,   color: 'text-blue-500 bg-blue-50' },
    { labelKey: 'dashboard.activeCustomers',  value: '48',    icon: Users,     color: 'text-green-600 bg-green-50' },
    { labelKey: 'dashboard.employees',        value: '24',    icon: UserCheck, color: 'text-purple-600 bg-purple-50' },
    { labelKey: 'dashboard.thisMonthRevenue', value: '৳3.2L', icon: BarChart3, color: 'text-[#ff6d29] bg-[#fff3eb]' },
  ];
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Monday, 26 May 2025 — {t('dashboard.welcome')}</p>
        </div>
        ...
        <div className="lg:col-span-2 bg-white border border-[#DBDFE9] rounded-xl p-5">
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-3 text-xs text-gray-500">
              ...
              <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-sm bg-[#DBDFE9] inline-block" /> {t('dashboard.purchase')}</span>
            </div>
          </div>
            ...
            {monthlyData.map((d) => (
              <div key={d.month} className="flex-1 flex flex-col items-center gap-0.5 group">
                <div className="relative w-full flex items-end gap-0.5 h-36">
                  <div className="flex-1 bg-[#DBDFE9] rounded-t transition-all duration-300 group-hover:bg-gray-300" style={{ height: `${d.purchase}%` }} title={`Purchase: ${d.purchase}%`} />
                  <div className="flex-1 bg-[#ff6d29] rounded-t transition-all duration-300 group-hover:bg-[#e65a1f]" style={{ height: `${d.sales}%` }} title={`Sales: ${d.sales}%`} />
                </div>
              ...
              return (
                <div key={item.sku}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xs font-bold text-red-500 bg-red-50 px-2 py-0.5 rounded-full">{item.current} {t('dashboard.left')}</span>
                  </div>
